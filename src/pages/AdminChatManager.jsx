  ...
  useEffect(() => {
    ...
    const triggerAutoReply = async (studentId, studentName, courseId) => {
      const autoMsg = {
        ...
        courseId: courseId,
        text: `Hello ${studentName}, thank you for contacting AYAX Academy Support. An administrator has been notified and will be with you shortly.`,
        sender: "admin", // System replies as admin
      ...
      try {
        await addDoc(collection(db, "chats"), autoMsg);
      } catch (error) {
    ...
    <div className="flex h-[85vh] bg-white rounded-[3rem] shadow-sm border border-gray-100 overflow-hidden m-4">
      ...
      <aside className="w-80 border-r border-gray-100 flex flex-col bg-gray-50/30">
        <div className="p-8 border-b border-gray-100">
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase mt-1 tracking-widest truncate">
            {courseId}
          </p>
        </div>
          ...
          <>
            ...
            <div className="flex-grow p-8 overflow-y-auto space-y-6 bg-gray-50/20">
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`max-w-[70%] p-5 rounded-[2rem] font-bold text-sm shadow-sm ${
                      msg.sender === "admin"
            ...
            <div className="text-left overflow-hidden">
              ...
              <div className="flex items-center gap-2">
                ...
                {/* If the last sender was admin (the bot), show a small robot icon or tag */}
                {student.sender === "admin" && (
                  <span className="text-[8px] bg-blue-100 text-blue-600 px-1 rounded uppercase font-black">
                    Bot
                  </span>
                )}
              </div>
            ...
            <form
              ...
              className="p-8 border-t border-gray-50 flex gap-4"
            >
              <input
                type="text"
                value={reply}
              ...
              <button
                type="submit"
                className="w-16 h-16 bg-blue-600 text-white rounded-2xl flex items-center justify-center hover:bg-blue-700 shadow-xl shadow-blue-100 transition-all active:scale-95 shrink-0"
              >
                <Send size={24} />
              </button>
            </form>
