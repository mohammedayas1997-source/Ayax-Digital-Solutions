...
const AdminCourseList = () => {
  ...
  // Updated static course list for reference or mapping if needed
  ...
  const deleteCourse = async (courseId, title) => {
    if (
      window.confirm(
        `Are you sure you want to delete ${title}? This action will remove all data associated with this course.`,
      )
    ) {
      try {
        ...
        {courses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4 mb-6">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {availableCourses.map((c, i) => (
                  <span
                    key={i}
                    className="px-3 py-1 bg-gray-100 rounded-full text-[8px] font-bold text-gray-500"
                  >
                    {c}
                  </span>
                ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {courses.map((course) => (
              <div
                key={course.id}
                className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-gray-100 hover:shadow-2xl hover:-translate-y-3 transition-all group relative"
              >
                <button
