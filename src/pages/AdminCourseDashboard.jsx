...
import { Users, HelpCircle, FileText, ChevronLeft, Bell, MessageSquare } from 'lucide-react';
...
const AdminCourseDashboard = () => {
  ...
  useEffect(() => {
    ...
    // 1. Live Listener na Enrollments
    ...
    // 2. Live Listener na Pending Submissions
    const examQuery = query(
      collection(db, "submissions"), 
      where("courseId", "==", courseId), 
      where("graded", "==", false) // Mun yi amfani da 'graded' tunda shi ne muke update a AdminGrading
    );
    ...
    // Cleanup listeners lokacin da aka fita daga shafin
    return () => {
      unsubEnroll();
      unsubExams();
    };
  ...
  const menuItems = [
    { name: 'Questions Bank', icon: <HelpCircle size={20} />, path: `/admin/questions/${courseId}` },
    { name: 'Student Grading', icon: <FileText size={20} />, path: `/admin/grading/${courseId}` },
    { name: 'Enrolled Students', icon: <Users size={20} />, path: `/admin/students/${courseId}` },
    { name: 'Course Chat', icon: <MessageSquare size={20} />, path: `/admin/chat/${courseId}` },
  ];
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="w-80 bg-white border-r border-gray-100 p-8 flex flex-col hidden lg:flex">
        ...
        >
          ...
          <span className="font-black text-[10px] uppercase tracking-widest">Back to Courses</span>
        </div>
        ...
        <div className="p-6 bg-gray-50 rounded-[2rem] mt-auto border border-gray-100">
          <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1">Active Context</p>
          <p className="text-sm font-black text-gray-800 truncate uppercase">{courseId.replace(/-/g, ' ')}</p>
        </div>
      </aside>
      ...
      <main className="flex-grow p-6 md:p-12">
        <header className="mb-12 flex justify-between items-start md:items-end">
          <div>
            <p className="text-blue-600 font-black uppercase text-[10px] tracking-[0.4em] mb-2">Management Suite</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-blue-600 p-10 rounded-[3.5rem] text-white shadow-2xl shadow-blue-200 relative overflow-hidden group">
            ...
            <h3 className="text-xl font-bold opacity-80 mb-1">Enrolled Students</h3>
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-sm relative overflow-hidden group">
            ...
            <h3 className="text-xl font-black text-gray-900 mb-1">Grading Queue</h3>
            <p className="text-7xl font-black text-blue-600 tracking-tighter">
              {loading ? '---' : stats.pendingExams}
            </p>
            <p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest mt-2">Unmarked Submissions</p>
            <button 
        ...
        <div className="mt-12">
            <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mb-6">Quick Actions</h4>
