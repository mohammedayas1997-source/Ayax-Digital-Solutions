    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        if (userData.role === "student") {
          ...
          await signOut(auth);
          alert(
            "RESTRICTED: This portal is for students only. Use the Admin Gateway.",
          );
        }
      } else {
        await signOut(auth);
        alert(
          "ACCOUNT ERROR: No student profile found. Please contact administration.",
        );
      }
    } catch (error) {
      console.error(error);
      alert(
        "AUTHENTICATION ERROR: Student record not found or invalid credentials.",
      );
    } finally {
      setLoading(false);
    }
